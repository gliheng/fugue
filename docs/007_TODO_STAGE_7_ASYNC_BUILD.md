# Stage 7: Async Build Pipeline + WebSocket Log Streaming

## Overview

Stage 7 adds asynchronous building and deployment. Currently, `fugue deploy` blocks the CLI while `npm install && npm run build` runs (up to 5 minutes for large projects). The new build pipeline runs builds in background tasks, streams logs via WebSocket, and tracks build state in PostgreSQL.

This also enables the web Dashboard (Stage 9) to show real-time build progress.
