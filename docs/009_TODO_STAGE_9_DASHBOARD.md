# Stage 9: Dashboard (React Router v7 on workerd)

## Overview

Stage 9 creates the web Dashboard — a React Router v7 application that runs on Fugue itself (self-hosting on workerd). The Dashboard provides:
...
| UI Components | shadcn/ui | Clean, professional design |
...
- Framework badge (React Router / Nuxt.js / Single File)
...
- Framework selector (React Router / Nuxt.js / Single File)
- Auto-deploy toggle
...
### `/settings` — Platform Settings
- AI provider config (API key, model)
- Domain config (fugue.local)
- workerd config (port, binary path)
...
- Stage 7 (Async Build) — Build log WebSocket
